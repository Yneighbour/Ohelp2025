...
- “新增/编辑用户”仍为前端弹窗演示，暂未接入 `POST/PUT` 接口。

**结论**  
- “用户列表视图”与“启用/禁用/删除”操作已对接后端；
- “新增/编辑用户”等表单写入能力已存在于后端，但前端仍为演示模式，可在需要时补充表单与接口调用逻辑。  
...
- “新增/编辑/删除/重新上架”等其他写操作仍为前端弹窗演示，未调用 `POST/PUT/DELETE` 接口。  
